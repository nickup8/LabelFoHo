...
const MATERIAL_MAP: Record<string, { code: string; label: string }> = {
    ПП: { code: '5', label: 'PP' },
    PP: { code: '5', label: 'PP' },
    ПВХ: { code: '3', label: 'PVC' },
    PVC: { code: '3', label: 'PVC' },
    PU: { code: '7', label: 'OTHER' },
    ПУ: { code: '7', label: 'OTHER' },
    ПОЛИУРЕТАН: { code: '7', label: 'OTHER' },
};

function resolveRecycling(composition: string): {
    code: string;
    label: string;
} {
    const upper = composition.toUpperCase();
    for (const [key, value] of Object.entries(MATERIAL_MAP)) {
        if (upper.includes(key)) return value;
    }
    const trimmed = upper.trim();
    if (/^3$/.test(trimmed) || /[\s,;\/]3[\s,;\/]/.test(` ${upper} `)) {
        return { code: '3', label: 'PVC' };
    }
    return { code: '7', label: 'OTHER' };
}
...
export default function RecyclingSign({
    ...
    const recycling =
        recycleCode && recycleLabel
            ? { code: recycleCode.replace(/^0+/, ''), label: recycleLabel }
            : composition
              ? resolveRecycling(composition)
              : { code: '7', label: 'OTHER' };
        ...
        <Svg width={62} height={62} viewBox="0 0 62 62">
            ...
            <Text
                ...
                style={
                    {
                        fontFamily: 'Roboto',
                        fontSize: recycling.label.length > 3 ? 10 : 12,
                        fontWeight: 700,
                    } as any
                }
            >
